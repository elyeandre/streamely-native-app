...
const providersObject = providers
  ...
  .addSource({
    ...
    flags: [],
    async scrapeMovie(ctx) {
      const baseUrl = 'https://pinoymoviepedia.ru';

      const searchResultRequest = await ctx.proxiedFetcher(`/wp-json/dooplay/search/`, {
        baseUrl: baseUrl,
        query: {
      ...
      if (searchResultRequest.error === 'no_posts') {
        throw new providers.NotFoundError('No watchable item found');
      }

      const searchResults = Object.values(searchResultRequest);

      const watchPageUrl = searchResults.find(
        (x) => x && compareTitle(ctx.media.title, x.title.replace(/\s*\([^)]*\)\s*.*$/, ''))
      )?.url;
      if (!watchPageUrl) throw new providers.NotFoundError('No watchable item found');

      const videoPage = await ctx.proxiedFetcher(watchPageUrl);

      const parser = new DOMParser();
      // Parse the HTML string
      const doc = parser.parseFromString(videoPage, 'text/html');
      // Now you can use standard DOM methods to access elements in the parsed HTML document
      const iframes = doc.querySelectorAll('.dooplay_player #dooplay_player_content iframe');
      // Initialize an array to store the src attributes
      const embeds = [];
      // Loop through each iframe and extract its src attribute
      iframes.forEach((iframe) => {
        const src = iframe.getAttribute('src');
        if (src) {
          // Push embed objects based on the source URLs
          if (src.includes('luluvdo')) {
            embeds.push({
              embedId: 'luluvdo',
              url: src
            });
          } else if (src.includes('mixdrop')) {
            embeds.push({
              embedId: 'mixdrop-nu',
              url: src
            });
      ...
      // Now embedLinks array contains all the src attributes of the iframes
  ...
  .addSource({
    ...
    async scrapeMovie(ctx) {
      ...
      const apiURL = 'https://elyeandre.vercel.app/movie';
      const m3u8Proxy = 'https://proxy.streamely.online/?destination=';
      const searchResultRequest = await ctx.proxiedFetcher.full(`/${tmdbId}`, {
        baseUrl: apiURL
      });
      if (searchResultRequest.statusCode === 404) {
        throw new providers.NotFoundError('No watchable item found');
      }
      console.log(
        `${m3u8Proxy}${searchResultRequest.body.stream?.playlist}&user-agent=Mozilla/5.0&accept-language=en-US,en;q=0.5`
      );
      return {
        embeds: [],
        stream: [
          {
            id: 'primary',
            captions: searchResultRequest.body.stream?.captions?.url || [],
            playlist: `${m3u8Proxy}${searchResultRequest.body.stream?.playlist}&user-agent=Mozilla/5.0&accept-language=en-US,en;q=0.5`,
            // playlist: searchResultRequest.body.stream?.playlist,
            type: 'hls',
            flags: [],
            headers: {
              'User-Agent': searchResultRequest.body.stream.headers['User-Agent'],
    ...
    async scrapeShow(ctx) {
      const tmdbId = ctx.media.tmdbId;
      const seasonNo = ctx.media.season.number;
      const episodeNo = ctx.media.episode.number;
      const apiURL = 'https://elyeandre.vercel.app/tv';
      const m3u8Proxy = 'https://proxy.streamely.online/?destination=';
      const searchResultRequest = await ctx.proxiedFetcher.full(`/${tmdbId}/${seasonNo}/${episodeNo}`, {
        baseUrl: apiURL
      });
      if (searchResultRequest.statusCode === 404) {
        throw new providers.NotFoundError('No watchable item found');
  ...
  .addEmbed({
    ...
    flags: [providers.flags.IP_LOCKED],
    async scrape(ctx) {
      const luluvdoBase = 'https://luluvdo.store';
      ...
      const packed = streamRes.match(evalCodeRegex);

      if (!packed) {
        throw new Error('failed to find packed luluvdo JavaScript');
      }

      const unpacked = unPack(packed[0]);
      const link = unpacked.match(fileRegex);

      if (!link) {
        throw new Error('failed to find packed luluvdo source link');
      }

      const url = link[0].slice(6, -1);
  ...
  .addEmbed({
    ...
    async scrape(ctx) {
      ...
      let embedUrl = ctx.url;
      embedUrl = (await ctx.proxiedFetcher.full(ctx.url)).finalUrl;
